...
struct ICBContainer
{
    ...
    render_pipeline_state render_pipeline_state [[ id(ArgumentBufferIDPipeline) ]];
};
...
//                                 constant uint *indices [[ buffer(KernelBufferIndexIndices) ]],
                                 ...
                                 device ComputeMaterial &material [[ buffer(KernelBufferIndexTextures) ]]
                                 )
{
    ...
    cmd.set_render_pipeline_state(icb_container->render_pipeline_state);
...
//    device Material &material = materials[vertexIn.textureIndex];
    texture2d<float> mainTexture = material.textures[vertexIn.textureIndex];
//
