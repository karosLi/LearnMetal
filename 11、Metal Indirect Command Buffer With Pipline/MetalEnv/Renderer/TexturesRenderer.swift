...
//    private var indexBuffer: MTLBuffer!
    ...
    private var instanceTextureArgumentBuffers: [MTLBuffer] = []
...
//            icbDescriptor.inheritPipelineState = true
        ...
        indirectCommandBuffer = device.makeIndirectCommandBuffer(descriptor: icbDescriptor, maxCommandCount: instances.count, options: .storageModePrivate)
        ...
        /// 把纹理都绑定到片元着色器上
        instanceTextureArgumentBuffers.removeAll()
        for (var index, texture) in textures.enumerated() {
            ...
            instanceTextureArgumentBuffers.append(fragmentTextureArgumentBuffer!)
        }
        ...
        computeTextureArgumentBuffer = device.makeBuffer(length: textureArgumentSize * instanceTextureArgumentBuffers.count, options: .storageModeShared)
        computeTextureArgumentBuffer.label = "Compute Textures Argument Buffer"
        
        for (index, buffer) in instanceTextureArgumentBuffers.enumerated() {
            computeTexturesArgumentEncoder.setArgumentBuffer(
                computeTextureArgumentBuffer, startOffset: 0, arrayElement: index)
            computeTexturesArgumentEncoder.setBuffer(buffer, offset: 0, index: 0)
        }
    }
...
extension TexturesRenderer: MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        guard let drawable = view.currentDrawable,
              ...
              let indirectCommandBuffer = indirectCommandBuffer,
